...
function MetaLabel({ category = "CORE", value = "0x01", status = "LIVE" }) {
  return (
    <div className="flex items-center gap-3 select-none">
      <div className="flex items-center rounded-[2px] overflow-hidden border border-border dark:border-white/10 shadow-sm">
        <span className="px-1.5 py-0.5 text-[8px] font-black bg-foreground dark:bg-card text-white dark:text-black uppercase tracking-tighter">
          {category}
        </span>
        <span className="px-1.5 py-0.5 text-[8px] font-mono text-muted-foreground dark:text-muted-foreground bg-accent/50 dark:bg-card/5">
          {value}
        </span>
      </div>
...
export function OurValues() {
  ...
  return (
    <section className="relative py-40 bg-card dark:bg-[#030303] transition-colors duration-700 overflow-hidden">
      {/* 1. 背景层：极细网格与边缘消散 */}
      ...
      <div className="container relative z-10 mx-auto px-6 lg:px-12">
        {/* 2. 头部：非对称工业排版 */}
        <div className="max-w-4xl mb-32 border-l-[1px] border-border dark:border-white/10 pl-8">
          <motion.div
          ...
          <h2 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.8] text-foreground dark:text-white uppercase">
            核心 <br />
        ...
        {/* 3. 支柱格栅：阶梯式无边界卡片 */}
        <div className="grid grid-cols-1 lg:grid-cols-3 border-y border-border dark:border-white/5 divide-y lg:divide-y-0 lg:divide-x divide-border dark:divide-white/5">
          {values.map((v, i) => (
            <motion.div
              key={v.id}
              ...
              className="group relative p-12 lg:p-16 transition-all hover:bg-accent dark:hover:bg-card/[0.01]"
            >
              {/* 卡片顶部标签 */}
              <div className="mb-12 flex justify-between items-start">
                <MetaLabel category="Pillar" value={v.id} />
                <v.icon className="w-5 h-5 text-muted-foreground dark:text-white/20 group-hover:text-primary transition-colors" />
              </div>

              {/* 内容区域 */}
              <div className="space-y-6">
                <div className="space-y-2">
                  <h3 className="text-2xl font-black tracking-tight text-foreground dark:text-white uppercase leading-none">
                    {v.title}
                  </h3>
                ...
                <p className="text-muted-foreground dark:text-muted-foreground text-sm leading-relaxed font-light">
                  {v.desc}
                </p>
              </div>
      ...
      <div className="absolute bottom-10 right-10 hidden lg:flex items-center gap-4 opacity-10">
        ...
        <div className="w-12 h-[1px] bg-card" />
      </div>
    </section> // <-- 完美闭合，且带边缘装饰
  );
}
