...
export function Achievements() {
  return (
    <section className="relative py-32 bg-white dark:bg-[#030303] transition-colors duration-700 overflow-hidden">
      {/* 背景：极细的扫描线 (Scanlines) */}
      ...
      <div className="container relative z-10 mx-auto px-6 lg:px-12">
        ...
        <div className="flex flex-col lg:flex-row items-end justify-between gap-8 mb-24">
          <div className="max-w-2xl text-left">
            ...
            <h2 className="text-5xl md:text-7xl font-black tracking-tighter leading-none text-slate-900 dark:text-white uppercase">
              数字背后的 <br />
              <span className="opacity-20 italic">底层逻辑</span>
            </h2>
          </div>
          <p className="text-slate-500 dark:text-zinc-500 text-sm max-w-[280px] border-l border-slate-200 dark:border-white/10 pl-6 mb-2">
...
function MetricItem({ item, index }: { item: any; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white dark:bg-[#030303] p-10 lg:p-12 transition-colors hover:bg-slate-50 dark:hover:bg-white/[0.02]"
    >
      {/* 装饰性索引编号 */}
      ...
      <div className="flex flex-col items-start space-y-6">
        <div className="space-y-1">
          <p className="text-[10px] font-bold tracking-[0.2em] text-zinc-500 dark:text-zinc-500 uppercase">
            {item.label}
          </p>
          <div className="flex items-baseline">
            <span className="text-6xl lg:text-7xl font-black tracking-tighter text-slate-900 dark:text-white">
              {item.value}
            </span>
